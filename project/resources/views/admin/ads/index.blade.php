...
@section('content')
<input type="hidden" id="headerdata" value="{{ __('ADVERTISEMENTS') }}">
<div class="content-area">
    <div class="mr-breadcrumb">
        <div class="row">
            <div class="col-lg-12">
                <h4 class="heading">{{ __('Advertisements') }}</h4>
                <ul class="links">
                    <li>
                        <a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }} </a>
                    </li>
                    <li>
                        <a href="{{ route('ads.index') }}">{{ __('Manage Advertisements') }}</a>
                    </li>
                </ul>
            </div>
        </div>
...
<div class="modal fade-scale" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="modal1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-center">
                    {{ __('You are about to delete this Ad.Everything under this ad will be deleted') }}.
                </p>
...
<script type="text/javascript">
    ...
    $(function() {
        $(".btn-area").append('<div class="col-sm-4 text-right">'+
            '<a class="add-btn" data-href="{{route('ads.create')}}" id="add-data" data-toggle="modal" data-target="#modal1">'+
            '<i class="fas fa-plus"></i> {{ __('Add New Ad') }}'+
            '</a>'+
            '</div>');
    });
</script>
