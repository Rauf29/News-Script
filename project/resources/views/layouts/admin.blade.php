												...
												<ul>
													...
													<li>
														<form method="POST" action="{{ route('admin.logout') }}" style="display:inline">
															@csrf
															<button type="submit" class="btn btn-link" style="padding:0;color:inherit"><i class="fas fa-power-off"></i> {{ __('Logout') }}</button>
														</form>
													</li>
												</ul>
											</div>
...
{{-- Global handler for delete modals to use POST --}}
<script>
$(document).on('click', '.delete[data-target="#confirm-delete"]', function() {
    $('#confirm-delete #deleteForm').attr('action', $(this).data('href'));
});
$(document).on('click', '.delete[data-target="#confirm-delete-option"]', function() {
    $('#confirm-delete-option #deleteFormOption').attr('action', $(this).data('href'));
});
$(document).on('click', '.bulk-delete', function(e) {
    var href = $('#confirm-delete-option #deleteFormOption').attr('action');
    if (href) {
        $('#confirm-delete-option #deleteFormOption').submit();
    }
});
</script>
