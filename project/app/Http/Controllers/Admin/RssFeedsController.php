...
class RssFeedsController extends Controller
{
    ...
    public function datatables(){
        ...
        return Datatables::of($datas)
                                ...
                                ->editColumn('auto_update',function(Rss $data){
                                    return $auto = $data->auto_update == 0 ? '<form method="POST" action="'.route('rss.feedUpdate',$data->id).'" style="display:inline">'.csrf_field().'<button type="submit" class="btn btn-sm btn-info"><i class="fas fa-sync-alt"></i>Update</button></form>':'<button class="btn btn-sm btn-success" disabled="disabled"><i class="fas fa-sync-alt"></i>Cronjob Updated</button>';
                                })
    ...
    public function store(Request $request){
        ...
        try {
            $feed = \Feeds::make($data->feed_url);
            $items = $feed->get_items();
            $i =0;
            foreach($items as $item):
                if($i==$data->post_limit){
                    break;
                }
            ...
            $msg = 'Feed saved but could not import posts: '.$e->getMessage();
    ...
    public function feedUpdate($id){
        ...
        try {
            ...
            Toastr::error('Could not update feed: '.$e->getMessage());
