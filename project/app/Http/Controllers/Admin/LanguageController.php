...
class LanguageController extends Controller
{
    ...
    public function datatables(){
        ...
        return Datatables::of($datas)
                            ->addColumn('action',function(Language $data){
                                ...
                                $default = $data->is_default == 1 ? '<a><i class="fa fa-check"></i> Default</a>' : '<form method="POST" action="'.route('admin.language.status',$data->id).'" style="display:inline">'.csrf_field().'<button type="submit" class="btn btn-link" style="padding:0">Set Default</button></form>';
   ...
   public function delete($id){
    if($id == 1)
    {
    return response()->json("You don't have access to remove this language.");
    }
    $data = Language::findOrFail($id);
    if($data->is_default == 1)
    {
    return response()->json("You can not remove default language.");            
    }

    if($data->posts->count()>0){
       return response()->json("Remove Posts first!"); 
    }
    if($data->polls->count()>0){
        return response()->json("Remove Polls first!"); 
    }
    if($data->albums->count()>0){
        return response()->json("Remove Albums first!"); 
    }
     if($data->galleries->count()>0){
       return response()->json("Remove galleries first!"); 
    }
    if($data->pages->count()>0){
       return response()->json("Remove Pages first!");  
    }
      if($data->rss->count()>0){
        return response()->json("Remove RSS first!"); 
    }
    if($data->widgets->count()>0){
       return response()->json("Remove widgets first!"); 
    }
    if (file_exists(base_path().'/../project/resources/lang/'.$data->file)) {
        unlink(base_path().'/../project/resources/lang/'.$data->file);
    }
