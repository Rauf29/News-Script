...
class PQuizController extends Controller
{
    ...
    public function store(Request $request){
        $rules = [
            ...
            'image_big' => 'image|mimes:jpeg,png,jpg,gif,svg,webp,heic',
            'description' => 'required',
            'category_id' => 'required',
            'question_title.*' => 'required',
            'question_photo.*' => 'image|mimes:jpeg,png,jpg,gif,svg,webp,heic',
            'result_title.*' => 'required',
            'result_photo.*' => 'image|mimes:jpeg,png,jpg,gif,svg,webp,heic',
        ];
    ...
    public function removepquestion($id){
        $data = PersonalityQuestion::findOrFail($id);

        if($data->answers->count()>0){
            foreach($data->answers as $answer){
                $ans = PersonalityAnswer::find($answer->id);
                @unlink('assets/images/panswer/'.$ans->answer_photo);
                $ans->delete();
            }
        }
        $data->delete();
    ...
    public function removepresult($id){
        $data = PersonalityResult::findOrFail($id);
        @unlink('assets/images/presult/'.$data->result_photo);
        $data->delete();
        return response()->json('Result Deleted Successfully');
    }

    public function removeanswer($id){
        $data = PersonalityAnswer::findOrFail($id);
